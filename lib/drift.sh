...
ca_compare_reports() {
    ...
    if ! jq -en --slurpfile old "$reference" --slurpfile new "$difference" '($old[0].scope == null or $new[0].scope == null) or (($old[0].scope | del(.level)) == ($new[0].scope | del(.level)))' >/dev/null; then ca_die 'compare scope differs; compare only the same assets and provider context'; fi
